...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const savedRecommendation = localStorage.getItem('birthControlRecommendation');
    ...
    const fetchData = async () => {
      try {
        ...
        const storedResponses = sessionStorage.getItem("finalBCResponses");
        ...
        const response = await fetch('/api/bc-recommendations', {
          method: 'POST',
          headers: {
        ...
        if (typeof data === 'string' || (data && !data.hasOwnProperty('eligible'))) {
          ...
          const explanation = typeof data === 'string' ? data : data.explanation || "Based on your responses, we cannot recommend a specific birth control at this time.";
        ...
        localStorage.setItem('birthControlRecommendation', JSON.stringify(parsedRecommendation));
  ...
  const startOver = () => {
    localStorage.removeItem('birthControlRecommendation');
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl mx-auto">
              ...
              <div className="p-6">
                <div className="bg-gray-50 p-6 rounded-lg mb-8">
                  <p className="text-lg leading-relaxed">{recommendation?.explanation || "Based on your responses, we recommend consulting with a healthcare provider for personalized birth control options. Your health is our priority."}</p>
                </div>
