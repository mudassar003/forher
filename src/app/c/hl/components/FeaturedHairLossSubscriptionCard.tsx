//src/app/c/hl/components/FeaturedHairLossSubscriptionCard.tsx
"use client";
...
interface FeaturedHairLossSubscriptionCardProps {
  className?: string;
}

const FeaturedHairLossSubscriptionCard: React.FC<FeaturedHairLossSubscriptionCardProps> = ({ 
  className = "" 
}) => {
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [featuredSubscription, setFeaturedSubscription] = useState<Subscription | null>(null);
  const [imageUrl, setImageUrl] = useState<string>('/images/hair-loss-product.jpg'); // Default fallback image
  ...
  useEffect(() => {
    const fetchFeaturedSubscription = async (): Promise<void> => {
      try {
        ...
        // If none is marked as featured, get the first hair care subscription
        ...
        console.error("Error fetching featured hair care subscription:", err);
      ...
      <div className={`bg-white rounded-2xl shadow-xl overflow-hidden ${className}`}>
        <div className="bg-gradient-to-r from-[#e63946] to-[#ff4d6d] p-6 text-white">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
            <h3 className="text-2xl font-bold">Hair Loss Treatment Subscription</h3>
            <span className="px-3 py-1 bg-white text-[#e63946] text-sm font-semibold rounded-full inline-block w-max">Coming Soon</span>
          </div>
          <p className="opacity-90 mt-1">Personalized hair restoration program</p>
        </div>
        ...
        <div className="p-6">
          <div className="mb-4 text-center">
            ...
            <h4 className="text-xl font-semibold text-black mb-2">No Hair Loss Plans Available Yet</h4>
            <p className="text-gray-600 mb-6">
              We're working on bringing you personalized hair loss treatment plans. Thank you for completing our assessment - your responses help us develop better solutions for your needs.
            </p>
          </div>
          ...
          <div className="space-y-3 mb-6">
            <div className="flex items-start gap-3">
              ...
              <span className="text-gray-700">Personalized hair loss treatment</span>
            </div>
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#f9dde5] flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#e63946" className="w-4 h-4">
              ...
              <span className="text-gray-700">Expert dermatologist support</span>
            </div>
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#f9dde5] flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#e63946" className="w-4 h-4">
...
export default FeaturedHairLossSubscriptionCard;