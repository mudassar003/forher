...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const savedRecommendation = localStorage.getItem('consultationRecommendation');
    
    const fetchData = async () => {
      try {
        // Get treatment interest for category filtering
        let treatmentCategory = "general";
        const storedResponses = sessionStorage.getItem("finalConsultationResponses");
        if (storedResponses) {
          const responses = JSON.parse(storedResponses);
          treatmentCategory = responses['treatment-interest'] || "general";
        }
        
        // Fetch all products for the selected category
        const products: Product[] = await client.fetch(`
          *[_type == "product" && references(*[_type=="productCategory" && slug.current=="${treatmentCategory}"]._id)] {
        ...
        const response = await fetch('/api/consult-recommendations', {
          method: 'POST',
          headers: {
        ...
        if (typeof data === 'string' || (data && !data.hasOwnProperty('eligible'))) {
          ...
          const explanation = typeof data === 'string' ? data : data.explanation || "Based on your responses, we cannot recommend our products at this time.";
        ...
        localStorage.setItem('consultationRecommendation', JSON.stringify(parsedRecommendation));
  ...
  const startOver = () => {
    localStorage.removeItem('consultationRecommendation');
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl mx-auto">
              ...
              <div className="p-6">
                <div className="bg-gray-50 p-6 rounded-lg mb-8">
                  <p className="text-lg leading-relaxed">{recommendation?.explanation || "Based on your responses, we recommend consulting with a healthcare provider before pursuing treatment. Your health is our priority."}</p>
                </div>
        ...
        <section>
          ...
          <p className="text-gray-600 max-w-3xl mb-8">Browse our selection of physician-formulated products designed to help with your specific needs.</p>
