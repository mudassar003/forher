...
const getMonthlyPriceDisplay = (
  ...
  return `${getSimplifiedPrice(monthlyPrice)}/mo`;
};
...
const getTotalPriceDisplay = (
  ...
  // Format based on billing period
  switch (billingPeriod) {
    case 'monthly':
      return `${getSimplifiedPrice(price)} total`;
    case 'three_month':
      return `${getSimplifiedPrice(price)} / 3 months`;
    case 'six_month':
      return `${getSimplifiedPrice(price)} / 6 months`;
    case 'annually':
      return `${getSimplifiedPrice(price)} / year`;
    case 'other':
      if (customBillingPeriodMonths && customBillingPeriodMonths > 1) {
        return `${getSimplifiedPrice(price)} / ${customBillingPeriodMonths} months`;
      } else {
        return `${getSimplifiedPrice(price)} total`;
      }
    default:
